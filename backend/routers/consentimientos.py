...
@router.post("/", response_model=schemas.Consentimiento)
...
@router.get("/", response_model=list[schemas.Consentimiento])
def listar_consentimientos(db: Session = Depends(database.get_db)):
    return crud.obtener_consentimientos(db)

@router.get("/{consentimiento_id}", response_model=schemas.Consentimiento)
...
@router.put("/{consentimiento_id}", response_model=schemas.Consentimiento)
def actualizar_consentimiento(consentimiento_id: int, consentimiento: schemas.ConsentimientoCreate, db: Session = Depends(database.get_db)):
    return crud.actualizar_consentimiento(db, consentimiento_id, consentimiento)
